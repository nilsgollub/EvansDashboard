...
def parse_maxspeed(speed_str):
    """Wandelt einen OSM 'maxspeed'-Tag in eine ganze Zahl (km/h) um.

    'none' (Schweizer Autobahn ohne Limit-Tag) -> 120. Nicht parsebare Werte -> None.
    """
    if speed_str is None:
        return None
    speed_str = str(speed_str).strip()
    if speed_str == "none":
        return 120  # Schweizer Autobahn-Standard
...
def get_speed_limit_online(lat, lon, radius=30):
    ...
    headers = {"User-Agent": "EvansDashboard/1.0 (Testprojekt für privates Dashboard)", "Accept": "application/json"}
    ...
    try:
        ...
        for element in elements:
            ...
            if "maxspeed" in tags:
                parsed = parse_maxspeed(tags["maxspeed"])
