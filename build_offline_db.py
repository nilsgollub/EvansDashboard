        ...
        def progress_callback(block_num, block_size, total_size):
            ...
            if total_size > 0:
                ...
                if int(percent) % 10 == 0 and int(percent - (block_size * 100 / total_size)) % 10 != 0:
                    logger.info(f"Download-Fortschritt: {percent:.1f}% ({read_so_far / (1024*1024):.1f} MB von {total_size / (1024*1024):.1f} MB)")
...
def build_database():
    ...
    class RoadSegmentHandler(osmium.SimpleHandler):
        ...
        def save_batch(self):
            ...
            conn.commit()
            logger.info(f"Fortschritt: {self.total_ways_processed:,} Wege verarbeitet, {self.total_segments_saved:,} Straßensegmente in DB gespeichert.")
            self.segments = []
    ...
    cursor.execute("INSERT OR REPLACE INTO metadata (key, value) VALUES (?, ?)", ("ways_processed", str(handler.total_ways_processed)))
    cursor.execute("INSERT OR REPLACE INTO metadata (key, value) VALUES (?, ?)", ("segments_saved", str(handler.total_segments_saved)))
